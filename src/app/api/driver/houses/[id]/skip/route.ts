...
import { skipHouseDto } from "@/core/dto/driver";
...
export const POST = withRoute<{ id: string }>(async ({ ctx, req, params }) => {
  ...
  const dto = skipHouseDto.parse(await req.json());
  return ok(await driverService.skipHouse(ctx.supabase, driverId, houseId, dto));
});
